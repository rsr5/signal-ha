...
/// Thread-safe SQLite store for entity state history.
pub struct Store {
...
/// Aggregate statistics for a numeric entity over a time range.
#[derive(Debug, Clone, Serialize)]
...
impl Store {
    ...
    /// Insert a state record. Silently ignores exact duplicates (same entity + timestamp + state).
    pub fn record(
        &self,
        entity_id: &str,
        state: &str,
    ...
    /// Query records for an entity within a time range (inclusive).
    pub fn query(
        &self,
        entity_id: &str,
        from: DateTime<Utc>,
    ...
    /// Query numeric state values for an entity in a time range.
    /// Rows whose state cannot be parsed as f64 are silently skipped.
    pub fn query_numeric(
        &self,
        entity_id: &str,
        from: DateTime<Utc>,
        to: DateTime<Utc>,
    ) -> Result<Vec<(DateTime<Utc>, f64)>, RecorderError> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
        ...
        for row in rows {
            let (state, ts) = row?;
            if let (Ok(val), Ok(dt)) = (state.parse::<f64>(), parse_ts(&ts)) {
                result.push((dt, val));
            }
        }
        Ok(result)
    ...
    /// Compute aggregate statistics for a numeric entity over a time range.
    pub fn stats(
        &self,
        entity_id: &str,
        from: DateTime<Utc>,
        to: DateTime<Utc>,
    ) -> Result<Option<NumericStats>, RecorderError> {
        let values = self.query_numeric(entity_id, from, to)?;
        if values.is_empty() {
            return Ok(None);
        }
    ...
    /// Delete records older than the given timestamp. Returns number of rows deleted.
    pub fn prune(&self, older_than: DateTime<Utc>) -> Result<u64, RecorderError> {
        let conn = self.conn.lock().unwrap();
    ...
    pub fn entities(&self) -> Result<Vec<String>, RecorderError> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare("SELECT DISTINCT entity_id FROM state_log ORDER BY entity_id")?;
        let rows = stmt
    ...
    pub fn latest(&self, entity_id: &str) -> Result<Option<Record>, RecorderError> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT entity_id, state, attributes, timestamp
             FROM state_log
    ...
    /// Count records for an entity (or all if entity_id is None).
    pub fn count(&self, entity_id: Option<&str>) -> Result<u64, RecorderError> {
        let conn = self.conn.lock().unwrap();
...
fn row_to_record(row: &rusqlite::Row<'_>) -> Result<Record, rusqlite::Error> {
    ...
    let timestamp = parse_ts(&ts).unwrap_or_else(|_| Utc::now());
    Ok(Record {
        entity_id,
        state,
        attributes,
...
mod tests {
    ...
    fn record_and_query() {
        ...
        store
            .record("sensor.temp", "21.5", None, ts(10, 0))
            .unwrap();
        store
            .record("sensor.temp", "22.0", None, ts(10, 30))
            .unwrap();
        store
            .record("sensor.temp", "22.3", None, ts(11, 0))
            .unwrap();
    ...
    #[test]
    fn query_numeric_skips_non_numeric() {
        let store = Store::open_in_memory().unwrap();
        store
            .record("sensor.temp", "21.5", None, ts(10, 0))
            .unwrap();
        store
            .record("sensor.temp", "unavailable", None, ts(10, 15))
            .unwrap();
        store
            .record("sensor.temp", "22.0", None, ts(10, 30))
            .unwrap();
        ...
        let values = store
            .query_numeric("sensor.temp", ts(10, 0), ts(10, 30))
            .unwrap();
        assert_eq!(values.len(), 2);
        assert!((values[0].1 - 21.5).abs() < f64::EPSILON);
        assert!((values[1].1 - 22.0).abs() < f64::EPSILON);
    }
    ...
    fn stats_aggregation() {
        let store = Store::open_in_memory().unwrap();
        store
            .record("sensor.temp", "20.0", None, ts(10, 0))
            .unwrap();
        store
            .record("sensor.temp", "24.0", None, ts(10, 30))
            .unwrap();
        store
            .record("sensor.temp", "22.0", None, ts(11, 0))
            .unwrap();

        let s = store
            .stats("sensor.temp", ts(10, 0), ts(11, 0))
            .unwrap()
            .unwrap();
        assert!((s.min - 20.0).abs() < f64::EPSILON);
        assert!((s.max - 24.0).abs() < f64::EPSILON);
        assert!((s.mean - 22.0).abs() < f64::EPSILON);
        assert_eq!(s.count, 3);
    }
    ...
    fn stats_empty_returns_none() {
        let store = Store::open_in_memory().unwrap();
        let s = store.stats("sensor.nope", ts(10, 0), ts(11, 0)).unwrap();
        assert!(s.is_none());
    }
    ...
    fn prune_removes_old_records() {
        let store = Store::open_in_memory().unwrap();
        store
            .record("sensor.temp", "20.0", None, ts(8, 0))
            .unwrap();
        store
            .record("sensor.temp", "21.0", None, ts(10, 0))
            .unwrap();
        store
            .record("sensor.temp", "22.0", None, ts(12, 0))
            .unwrap();
    ...
    fn entities_lists_distinct() {
        ...
        store
            ...
            .unwrap();
        store
            .record("light.porch", "on", None, ts(10, 0))
            .unwrap();
        store
            .record("sensor.temp", "22.0", None, ts(10, 30))
            .unwrap();
    ...
    fn latest_returns_most_recent() {
        ...
        store
            ...
            .unwrap();
        store
            .record("sensor.temp", "23.0", None, ts(12, 0))
            .unwrap();
